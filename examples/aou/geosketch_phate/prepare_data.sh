...
#   2. Run examples/aou/10k_WBH/config.yaml          (produces PCA for AoU samples)
#
...
if [[ ! -f "${PCA_CSV}" ]]; then
    ...
    echo "        examples/aou/10k_WBH/config.yaml"
    echo ""
...
# same coordinate space that geosketch ran in.
#
...
# config embeds the sketch exactly as the 10k_WBH example embeds its own subset,
# so that how the fit subset was chosen does not change how it is embedded.
#
...
echo "  bash examples/_shared/submit_batch.sh examples/aou/geosketch_phate/config.yaml"
echo ""
