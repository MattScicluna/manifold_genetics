...
if [[ "$SKIP_WRAYNER" != "true" ]]; then
    ...
    if [[ ! -f "${REFERENCE_HARMONIZED}.bed" ]]; then
        ...
        perl "${ABS_WRAYNER_SCRIPT}" \
            -b "reference_nochr.bim" \
            -f "reference_freq.frq" \
            -r "${ABS_TOPMED_REF}" \
            -l "${ABS_PLINK}" \
            -g
        ...
        mv ${REFERENCE_WRAYNER_DIR}/Strand-Flip* ${REFERENCE_WRAYNER_DIR}/Position* ${REFERENCE_WRAYNER_DIR}/Chromosome* \
           ...
           ${REFERENCE_WRAYNER_DIR}/FreqPlot* ${REFERENCE_WRAYNER_DIR}/Force-Allele1* ${REFERENCE_WRAYNER_DIR}/Exclude-* \
           ${REFERENCE_WRAYNER_DIR}/wrayner_tmp/ 2>/dev/null || true

        # Find the -updated output files from WRayner/Run-plink.sh
        UPDATED_BIM=$(ls ${REFERENCE_WRAYNER_DIR}/*-updated.bim 2>/dev/null | head -1)
        if [[ -n "${UPDATED_BIM}" ]] && [[ -f "${UPDATED_BIM}" ]]; then
            UPDATED_PREFIX="${UPDATED_BIM%-updated.bim}-updated"
            echo "    Using WRayner-updated files: $(basename ${UPDATED_PREFIX})"
            ...
            # Create final harmonized output
            cp "${UPDATED_PREFIX}.bed" ${REFERENCE_HARMONIZED}.bed
            cp "${UPDATED_PREFIX}.bim" ${REFERENCE_HARMONIZED}.bim
            cp "${UPDATED_PREFIX}.fam" ${REFERENCE_HARMONIZED}.fam
        else
            echo "    No -updated files found, using filtered data directly..."
            # Rename SNP IDs to chr:pos:ref:alt format
            echo "    Standardizing SNP IDs to chr:pos:ref:alt format..."
            cp ${REFERENCE_FILTERED}.bim ${TEMP_DIR}/reference_wrayner_filtered.bim.bkp
            awk '{print $1"\tchr"$1":"$4":"$6":"$5"\t"$3"\t"$4"\t"$5"\t"$6}' \
                ${TEMP_DIR}/reference_wrayner_filtered.bim.bkp > ${TEMP_DIR}/reference_wrayner_filtered.bim

            cp ${REFERENCE_FILTERED}.bed ${REFERENCE_HARMONIZED}.bed
            cp ${TEMP_DIR}/reference_wrayner_filtered.bim ${REFERENCE_HARMONIZED}.bim
            cp ${REFERENCE_FILTERED}.fam ${REFERENCE_HARMONIZED}.fam
        fi
