#!/bin/bash
#
# Generic Subset Pipeline Script
#
# This script runs the complete subset analysis pipeline:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: $0 --fit-plink PATH --project-plink PATH --fit-labels PATH --project-labels PATH --colormap PATH --output-dir PATH [OPTIONS]"
            exit 1
            ;;
    esac
done
...
# If --colormap provided, use it for both fit and project
if [[ -n "$COLORMAP" ]]; then
    FIT_COLORMAP="$COLORMAP"
    PROJECT_COLORMAP="$COLORMAP"
fi
...
if [[ -z "$FIT_PLINK" ]] || [[ -z "$PROJECT_PLINK" ]] || [[ -z "$FIT_LABELS" ]] || [[ -z "$PROJECT_LABELS" ]] || [[ -z "$FIT_COLORMAP" ]] || [[ -z "$PROJECT_COLORMAP" ]]; then
    ...
    echo "    --colormap PATH                      # Colormap JSON (for both fit and project)"
    echo "    OR"
    ...
    echo "    [--n-landmark N]                     # Number of landmarks for PHATE"
    echo "    [--random-landmarking]               # Use random landmarking"
    echo "    [--embedding-input fit|all]          # Embedding input (default: all)"
...
echo "  SNPs: $SNP_COUNT SNPs"

# ============================================================================
# Step 4: Run Pipeline
# ============================================================================
print_status "Running subset analysis pipeline..."
...
[[ -n "$N_LANDMARK" ]] && echo "    --n-landmark ${N_LANDMARK} \\"
[[ "$RANDOM_LANDMARKING" == "true" ]] && echo "    --random-landmarking \\"
[[ -n "$EMBEDDING_INPUT" ]] && echo "    --embedding-input ${EMBEDDING_INPUT} \\"
...
[[ -n "$EMBEDDING_INPUT" ]] && CMD="$CMD --embedding-input $EMBEDDING_INPUT"
...
echo ""
echo "================================================="
print_success "Subset pipeline analysis complete!"
echo "================================================="
echo ""
