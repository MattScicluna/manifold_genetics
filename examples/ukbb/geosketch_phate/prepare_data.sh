#!/bin/bash
#
# UKBB Geosketch PHATE - Data Preparation
#
# Selects a geometrically representative fit subset from the intersected UKBB
...
#   2. Run examples/ukbb/hgdp_1kgp_proj/run_pipeline.sh  (produces PCA for UKBB samples)
#
# The PCA CSV used for geosketch is the HGDP-projected UKBB PCA output:
#   examples/ukbb/hgdp_1kgp_proj/outputs/pca/transform_pca_20.csv
#
# Usage:
#   bash examples/ukbb/geosketch_phate/prepare_data.sh
#
# Override defaults via environment variables:
#   N_SKETCH=50000 bash examples/ukbb/geosketch_phate/prepare_data.sh
#   PCA_CSV=/path/to/pca.csv bash examples/ukbb/geosketch_phate/prepare_data.sh
#
...
# Source intersected UKBB data (from hgdp_1kgp_proj)
INTERSECTED_UKBB="${SCRIPT_DIR}/../hgdp_1kgp_proj/data/project_subset"

# PCA CSV for geosketch: HGDP-projected PCA coordinates for UKBB samples
...
print_header "UKBB Geosketch PHATE Data Preparation"
...
if [[ ! -f "${INTERSECTED_UKBB}.bed" ]]; then
    print_error "Intersected UKBB PLINK data not found!"
    echo "  Expected: ${INTERSECTED_UKBB}.bed"
    echo ""
    echo "  Please run the cross-projection pipeline first:"
    echo "    bash examples/ukbb/hgdp_1kgp_proj/prepare_data.sh"
    exit 1
fi
...
if [[ ! -f "${PCA_CSV}" ]]; then
    ...
    echo "        examples/ukbb/hgdp_1kgp_proj/run_pipeline.sh"
    echo ""
...
if [[ ! -f "${UKBB_LABELS_SOURCE}" ]]; then
    print_error "UKBB labels not found: ${UKBB_LABELS_SOURCE}"
    echo "  Please run examples/ukbb/hgdp_1kgp_proj/prepare_data.sh first."
    exit 1
fi

UKBB_SAMPLES=$(get_sample_count "${INTERSECTED_UKBB}")
COMMON_SNPS=$(get_snp_count "${INTERSECTED_UKBB}")

print_success "Found intersected UKBB data:"
echo "    Samples: $UKBB_SAMPLES"
...
if [[ -f "$FIT_SAMPLES_FILE" ]]; then
    ...
    python3 "${PROJECT_ROOT}/examples/aou/shared/select_samples_geosketch.py" \
        --pca "${PCA_CSV}" \
        --fam "${INTERSECTED_UKBB}.fam" \
...
source_labels = pd.read_csv("${UKBB_LABELS_SOURCE}")
if 'sample_id' not in source_labels.columns:
    # Handle manylatents-style column name
    for col in source_labels.columns:
        if 'sample' in col.lower() or 'id' in col.lower():
            source_labels = source_labels.rename(columns={col: 'sample_id'})
            break
source_labels['sample_id'] = source_labels['sample_id'].astype(str)
...
labels = source_labels[source_labels['sample_id'].isin(subset_samples)].copy()
labels.to_csv("${LABEL_FILE}", index=False)
print(f"    Created {len(labels)} labels with columns: {', '.join(labels.columns)}")
EOF
done
...
echo "  project_subset.{bed,bim,fam} (Full UKBB dataset: $PROJECT_SAMPLES samples, $FINAL_SNPS SNPs)"
